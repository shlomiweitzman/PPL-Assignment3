...
import { applyEnv, applyEnvBdg, globalEnvAddBinding, makeExtEnv, setFBinding,
            theGlobalEnv, Env, persistentEnv, isGlobalEnv, isExtEnv, isEnv, unbox, Frame, FBinding, generateBodyId, BodyId, EnvId } from "./L4-env-box";
...
const applicativeEval = (exp: CExp | Error, env: Env): Value | Error =>
    ...
    isLetExp(exp) ? evalLet(exp, env, env.envId) :
...
const applyProcedure = (proc: Value | Error, args: Array<Value | Error>, retEnv: EnvId): Value | Error =>
    isError(proc) ? proc :
    !hasNoError(args) ? Error(`Bad argument: ${getErrorMessages(args)}`) :
    isPrimOp(proc) ? applyPrimitive(proc, args) :
    isClosure(proc) ? applyClosure(proc, args, retEnv) :
    Error(`Bad procedure ${JSON.stringify(proc)}`);

const applyClosure = (proc: Closure, args: Value[], retEnv: EnvId): Value | Error => {
    let vars = map((v: VarDecl) => v.var, proc.params);
    return evalExps(proc.body, makeExtEnv(vars, args, proc.env, retEnv));
}
...
const evalLet = (exp: LetExp, env: Env, retEnv: EnvId): Value | Error => {
    const vals = map((v: CExp) => applicativeEval(v, env), map((b: Binding) => b.val, exp.bindings));
    const vars = map((b: Binding) => b.var.var, exp.bindings);
    if (hasNoError(vals)) {
        return evalExps(exp.body, makeExtEnv(vars, vals, env, retEnv));
    } else {
...
export const drawEnvDiagram = (pEnv: {}): Tree | Error => {
    ...
    // let lastEnv = envs[envs.length-2];
    // let g:Tree = makeTree()
    Object.values(pEnv).forEach(env => {
        if (isEnv(env)) addEnvToGraph(env, g)
    });
    return makeTreeFromGraph(g);
}

const addEnvToGraph = (env: Env, g: Graph): void => {
    let label: string ="{" + env.envId + "|" + frameToString(env, g) + "\\l}";
    let shape: string = "Mrecord";
    g.setNode(env.envId, { label, shape });
}

const frameToString = (env: Env, g: Graph): string =>{
    let frame: Frame;
    const bindingToStringWithEnvAndG = (binding: FBinding): string => bindingToString(binding, env, g);
    if (isGlobalEnv(env)){
        frame = unbox(env.frame);
        return frame.fbindings.map(bindingToStringWithEnvAndG).join("\\l|");
    }
    else if (isExtEnv(env)) {
        addEdgeToGraph(g, env.envId, env.returnValueEnv, "dashed");
        addEdgeToGraph(g, env.envId, env.env.envId);
        frame = env.frame;
        return frame.fbindings.map(bindingToStringWithEnvAndG).join("\\l|");
    }
    return "";
...
const addEdgeToGraph = (g: Graph, from: string, to: string, style?: string): void => {
    if (to.includes("_link")){
        const label = to.slice(0,to.search("_link"));
        g.setNode(to,{label, shape: "plaintext"});
    }
    style === undefined ? g.setEdge(from, to) : g.setEdge(from, to, {style});
}

const bindingToString = (binding: FBinding, env:Env, g: Graph): string => {
    const v: Value = unbox(binding.val);
    if (isClosure(v)){
        const bodyId: string = generateBodyId();
        addClosureToGraph(v, g, bodyId);
        g.setEdge(env.envId, bodyId, {tailport: binding.var, headport: 0})
        return "<" + binding.var + ">" + binding.var + ":";
    }
    return "<" + binding.var + ">" + binding.var + ":" + v;
}

const addClosureToGraph = (closure: Closure, g: Graph, id: BodyId): void => {
    let label: string = "{<0>◯◯\\l|p:" + closure.params.map((x:VarDecl) => x.var).join(",") + "\\l|b: " + closure.body.map(unparse) +"\\l}";
    let shape: string = "record";
    let color: string = "white";
    g.setNode(id, {label, shape, color});
    g.setEdge(id, closure.env.envId, {tailport: 0})
}

export const evalParseDraw = (s: string): string | Error => {
    evalParse(s);
    return (safeF(astToDot)(drawEnvDiagram(persistentEnv)));
}

const makeTreeFromGraph = (g: Graph): Tree => {
    return {tag: "Tree", rootId:"", graph: g}
}