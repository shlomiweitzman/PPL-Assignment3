...
export interface ExtEnv {
    ...
    env: Env;
    envId: EnvId; // added
    returnValueEnv: EnvId; //added
};
export const isExtEnv = (x: any): x is ExtEnv => x.tag === "ExtEnv";
export const makeExtEnv = (vs: string[], vals: Value[], env: Env, returnValueEnv?: EnvId): ExtEnv => {
    let returnEnv: EnvId;
    returnValueEnv === undefined ? returnEnv =  generateLinkId("GE_link") : returnEnv = generateLinkId( returnValueEnv + "_link");
    let ExtendedEnvironment : ExtEnv =
        ({tag: "ExtEnv", frame: makeFrame(vs, vals), env: env, envId: generateEnvId(), returnValueEnv: returnEnv});
    persistentEnv[ExtendedEnvironment.envId] = ExtendedEnvironment;
...
export const makeGlobalEnv = (): GlobalEnv => {
    let GlobalEnvironment : GlobalEnv = ({tag: "GlobalEnv", frame: makeBox(makeFrame([], [])), envId: "GE"});
    persistentEnv["GE"] = GlobalEnvironment;
...
let IdCounter: Box<number> = makeBox(0);
const generateLinkId = (s:string): EnvId => {
    let currentId = unbox(IdCounter);
    setBox(IdCounter, currentId + 1);
    return s + currentId;
}